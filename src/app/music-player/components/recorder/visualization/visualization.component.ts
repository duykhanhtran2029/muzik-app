...
export class VisualizationComponent implements OnInit {
  ...
  @ViewChild('ngAnimationContainer', { static: true })
  ngAnimationContainer: ElementRef;
  ...
  prepVisualization2() {
    ...
    for (let i = 0; i < TOTAL_BAR; i++) {
      this.appendRect(
        ...
        1,
        barWidth * i + 5 * i,
        this.containerHeight - barHeight / 2
      );
    }
  }
  ...
  prepAnimationCanvas() {
    this.svg = d3
      .select(this.ngAnimationContainer.nativeElement)
      .append('svg')
  ...
  visualize(source, stream, analyser) {
    ...
    let draw2 = function () {
      ...
      for (let i = 0; i < TOTAL_BAR; i++) {
        ...
        let newHeight =
          baseHeight + containerHeight * 0.9 * (dataArray[i] / 256);
